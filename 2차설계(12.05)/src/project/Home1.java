...
public class Home1 extends JPanel {
    public Home1(JFrame frame) {
        ...
        descriptionLabel.setBounds(170, 120, 400, 30); // "본체" 아래에 위치 설정
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                ...
                JButton button = new JButton("Button " + buttonIndex) {
                    @Override
                ...
                button.setBorder(BorderFactory.createLineBorder(new Color(139, 169, 152), 6)); // 테두리 색상 및 굵기 (4px)
                button.setFocusPainted(false); // 포커스 페인팅 제거

                // 버튼 클릭 이벤트
                button.addActionListener(e -> JOptionPane.showMessageDialog(this, "Button " + buttonIndex + " clicked!"));
        ...
        scrollPane.setBounds(160, 200, 530, 450); // 테이블 위치와 크기 설정 (크기를 키움)
